...
if hint == "yes":
    print(f'Hint - Word is starting >> {chosen_word[0]} ... and ending ... {chosen_word[-1:]} << \nlength is {len(chosen_word)}')
    display = ["_" for x in chosen_word]
    display[0] = chosen_word[0]; display[-1:] = chosen_word[-1:]
    print(f"{display}")
elif hint == "no":
    print("Ok good luck")
else:
...
while not end_of_game:
    ...
    if guess in display and display.count(guess) == chosen_word.count(guess):
        sm = display.count(guess) + chosen_word.count(guess)
        if display.count(guess) == chosen_word.count(guess) and sm != 0:
